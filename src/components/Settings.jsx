...
const Settings = () => {
  ...
  return (
    <div className="p-6 md:p-12 max-w-5xl mx-auto animate-in fade-in duration-1000">
      <header className="mb-14 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-5xl md:text-6xl font-black text-slate-900 tracking-tighter">⚙️ Settings</h2>
          <p className="text-2xl text-slate-400 font-medium mt-3">Customize your NeuroAssist experience.</p>
        </div>
        <div className="flex gap-4 flex-wrap">
          <button onClick={resetAll} className="flex items-center gap-3 px-7 py-4 rounded-2xl border-2 border-slate-200 font-black text-xl text-slate-600 hover:bg-slate-50 transition-all">
            <RotateCcw size={20} /> Reset
          </button>
          <button onClick={saveAll} className={`flex items-center gap-3 px-10 py-4 rounded-2xl font-black text-xl text-white shadow-xl transition-all ${saved ? 'bg-emerald-600 shadow-emerald-200' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-200 hover:scale-105'}`}>
