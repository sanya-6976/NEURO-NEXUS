...
const Voice = ({ setVoiceUsageCount, setLastMessage, setUserStatus }) => {
  ...
  const buildRecognition = useCallback(() => {
    ...
    return rec;
  }, [isListening, setLastMessage, setVoiceUsageCount, setUserStatus]);
  ...
  if (!supported) {
    return (
      <div className="p-10 max-w-3xl mx-auto text-center">
        ...
        <h2 className="text-4xl font-black text-slate-900 mb-4">Browser Not Supported</h2>
        <p className="text-xl text-slate-500 font-medium">
          Voice recognition requires Chrome or Edge browser. Please switch browsers to use this feature.
        </p>
      </div>
    );
  }
    ...
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col items-center gap-6 py-8">
        <button
          ...
          className={`relative w-48 h-48 rounded-full font-black text-white shadow-2xl transition-all duration-300 focus:outline-none focus:ring-8 flex flex-col items-center justify-center gap-3 ${
            isListening
        ...
        <div className={`flex items-center gap-3 px-6 py-3 rounded-full font-black text-lg border-2 transition-all ${
          ...
          {isListening ? '🔴 Listening... Keep speaking!' : '⬜ Stopped — Tap to start'}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {QUICK_PHRASES.map(({ label, phrase }) => (
            <button
              ...
              className="p-5 rounded-2xl bg-white border-2 border-slate-100 hover:border-blue-300 hover:bg-blue-50 font-black text-xl text-slate-700 transition-all hover:scale-105 shadow-sm text-left"
            >
              {label}
            </button>
          ))}
