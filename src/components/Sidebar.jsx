    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-80 bg-slate-900 text-white flex flex-col h-full shadow-2xl transition-transform duration-500 ease-in-out lg:relative lg:translate-x-0 ${
        ...
        <div className="p-8 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold tracking-wider mb-2">{t('app.title')}</h1>
            <p className="text-gray-400 text-lg uppercase tracking-widest font-semibold">{t('app.subtitle')}</p>
          </div>
          <button 
            onClick={toggle}
        ...
        <nav className="flex-1 px-4 py-6 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
          {navItems.map((item) => (
            <NavLink
              key={item.name}
              to={item.path}
        ...
        <div className="p-4 border-t border-white/5 space-y-2">
          <NavLink 
            ...
            onClick={() => { if (window.innerWidth < 1024) toggle(); }}
            className={({ isActive }) =>
              `w-full flex items-center space-x-4 px-5 py-3 text-base font-medium rounded-xl transition-all hover:text-white focus:outline-none focus:ring-4 focus:ring-amber-400 group ${
          ...
          {onLogout && (
            <button 
              onClick={onLogout}
              className="w-full flex items-center space-x-4 px-5 py-3 text-base font-medium text-red-400 hover:bg-red-500/10 rounded-xl transition-all hover:text-red-300 focus:outline-none focus:ring-4 focus:ring-red-400 group"
              aria-label="Logout"
            >
